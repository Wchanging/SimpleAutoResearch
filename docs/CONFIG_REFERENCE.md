...
| `[run].debug_artifacts` | When `true`, keeps verbose search diagnostics, retrieval-selection rows, coverage/section tables, debug cards, and legacy evidence-pack diagnostics. Keep it `false` for compact default runs; operational artifacts are still retained under their owning stages: `02-search` documents/chunks, `03-read` review/Paper Briefs, `04-synthesize` synthesis brief/Markdown, and `05-design` experiment contracts. |
