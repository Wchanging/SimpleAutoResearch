...
- 任务规划校验器与 LLM prompt 现在共用一份 planning boundary。默认步骤不再被当作进程授权：
  研究设计被接受前，已配置动作标为 deferred；survey 和没有执行配置的 research 进程动作标为 unauthorized。
  设计完成后仍只由现有协议步骤物化器提供可执行动作及条件，一次纠正会汇总提案中的边界错误。

- 研究请求构造现在只复用已有的裸 Python argv 解释策略，不探测或启动外部解释器；环境版本探测仍留在明确的 CodeTask 初始化/探查边界，绝对命令和非 Python 命令保持不变。研究配置与工作流文档同步为真实字段、续接归属、独立任务示例和能力驱动恢复，不再描述已删除的模板/固定阶段。

- 持续学习案例可直接运行仓库内配置：CodeTask TOML 支持显式 `{config_dir}` 案例路径，
  并在开始工作前检查声明的必要文件；任务资源和解释器写在 task TOML，`.env` 只保留全局
  凭据/模型/传输设置，旧配置的 cwd 相对路径语义不变。

- 科研方案身份与实验比较条件分离：基线复用和结果比较共用协议投影，修改假设或候选描述不再误判对照协议变化。旧完整合同只读重算比较，不改写历史记录；缺失或真实变化的条件仍保持未验证或不匹配。
  显式修订任务时，即使文字相同也重新检查资产；保留未变化输入的有效结果，数据变化则使相关测量失效。

- 固定命令的种子及明确配置的实验条件贯通设计与测量；扩展种子时替换已有参数，不重复追加。研究 CodeTask 准备阶段保留外部解释器策略，静态依赖检查使用指定解释器，不再用框架宿主环境误报训练依赖缺失。配置声明不等于独立验证，也不会追改历史实验事实。
- 实验报告优先使用已保存的执行合同，正文呈现声明的关键指标并链接详细测量，不再重复铺开每个任务的全部数值；派生差值与直接测量区分，报告工具及审计仍保留完整证据。

- 中英文 README 改为面向使用者的项目首页，围绕任务、真实调研快速开始、案例、产物与续跑组织；移除首页内部阶段交接说明，明确费用、准备条件与质量边界。
- examples 收敛为综述、持续学习研究、Digits 改码和多文件 Review 改码四个完整案例；测试依赖材料移入 tests/fixtures，离线会话 smoke 移入 scripts。删除不再使用的历史 GPU/greenfield/MCP 演示，同步调整当前文档和测试引用；历史 changelog 保留原记录。
- 设计澄清与 CodeTask 共用有界只读源码检索，补读过程保存在 attempt 内，服务异常时也保留；接入已有阅读与综合材料，区分自主实验选择与复现事实，可在不改变对照协议及用户固定候选的前提下重选已有候选。分析、报告和确认边界统一引用最近完成的有效设计，不再始终引用初始设计。
- 明确续跑已暂停的阻塞步骤时，重新尝试待完成动作，不再无限恢复旧阻塞结果；保留历史产物、有效测量和预算计数，意外中断的结果恢复与明确重试分别处理。

## 2026-09-25

- 实现返回明确设计缺口时，可在既有科研迭代上限内细化设计并创建新 CodeTask 工作区续接，不覆盖冻结输入或重跑有效基线；材料不足及重复缺口仍明确停止，中断保留已接受的细化步骤。baseline 未单独指定 cwd 时继承准备后的工作区。
- CodeTask 空提案索要缺失源码时，允许一次有界只读补充后重试，不扩大修改权限；仍不能完成时显示真实阻断原因，不再错误提示重新启用已启用的 LLM。研究实现 attempt 保留提案及补读材料，便于诊断与续接。
- 实验进度中的回车刷新改为 Rich 动态行，不再逐帧刷屏；按输出流拼接跨块日志，保留普通日志与原始进程记录，重定向输出省略临时刷新帧。
- 任务规划由框架派生 capability/state 路由，模型只选择动作并说明目的；无效计划最多回传纠正一次，不再静默替换为固定计划。attempt 下的 `task_plan_proposals.json` 保存返回提案及校验错误，历史 accepted plan 仍严格校验。Rich 使用通用任务规划说明，尚无交付时显示诊断目录而非空表格。
- 统一任务规划中 `summary` 能力名与 `summarize` 动作名的明确别名；CodeTask 准备后使用已生成的执行配置，不再把工作区创建和文件范围参数误传给改码接口。未知动作与能力边界仍严格校验。
- 在现有答复产物中保存决定对应的 brief/执行/报告修订值；答复落盘后中断可按原值恢复，同一决定 ID 携带不同修订值会被拒绝。报告与重分析入口不能绕过待处理的事实输入或交互决定。
...
- 新会话默认 `checkpoints`，并支持 `assisted`、`checkpoints`、`autonomous` 三种参与策略；关键事实/权限缺口仍始终暂停。`research-session` CLI/TOML 可对持久化决定进行接受、拒绝或修订，Rich 显示对应续接命令；提案与答复使用分开的决策产物，精确重放不会重复已完成工作。当前只有本地定向控制流验证，真实任务与冻结验收仍待进行。
...
- 将 V2.8 以准备好项目后的端到端主链走通作为阶段基线收口，并在 README
  明确验收范围：历史运行包含框架修复与恢复，不代表严格冻结验收或论文质量全部通过。
...
- 规划调用未显式设置阶段上限时继承统一模型输出上限，移除默认 1,200 token 对
  推理模型的隐式限制，避免思考耗尽额度后无法返回最终 JSON。
- 设计提示明确执行协议字段类型、seed/pairs 的互斥形式和省略未使用字段的规则；
  不通过放松既有校验或执行授权来接受模型输出。
- 明确继续时重试没有业务产物的失败调用，不再反复恢复同一失败；已测结果保持复用。
- 分析上下文压缩保留 canonical 基线/候选对照、来源和可比性限制；条件表为空也不丢弃
  已有实测对照，不把协议元数据不足误写成缺少基线。
- 增加可选的 Chat Completions 流式传输（`SIMPLE_AR_LLM_STREAM=true`），统一拼接 chunks
  并保留服务商 usage；Responses 仍使用非流式调用。

- GLM digits 诊断会话经修复与续跑完成设计、隔离改码、真实 baseline/candidate 实验和分析，
  重载没有重复执行。不等于 B 完整验收：分析中的协议与实现证据仍不完整，一次模型审阅
  超时，真实多 seed/reuse/skip 覆盖仍待完成；此前 provider 524 会话保持失败记录。
...
- 研究报告输入现在为当前选中的论文统一分配简短的 `P1`、`P2` 等引用键，并让 Writer、报告工具和组装器共享同一映射，避免有效的模型引用被误删为未知来源。

## 2026-09-15

- 候选代码经过修复时，报告现在会保留完整的实现 lineage：Writer/Reviewer
  同时收到初始 patch 和各次修复增量，不会把最后一次修复差异误当成完整改动。

- 报告 Writer/Reviewer 现在区分任务或准备上下文中声明的硬件、运行时条件与
  实际执行证据；当运行记录没有硬件信息时，不会把任务指定的加速卡写成已测事实。

- 实验报告现在默认生成有实测来源的确定性配对测量图；设置
  `[report.figures] enabled = false` 或 `mode = "off"` 仍可显式使用纯文本输出。
  自动配对报告最多选择四个代表性指标，不再为每个任务指标生成一张图。
  有证据链接的“假设未被支持”结果不再被报告审计误报为无依据主张。

- 如果可选的模型方案比较返回越界证据链接或暂时不可用，已有
  `ready` 候选现在会通过明确记录的确定性执行就绪回退继续；没有可执行候选时仍会暂停。

- `research-report --refresh` 基于已有测量重新分析并写入新报告 attempts，不重训、不删除旧稿；模型研究会话接回已有 LLM 假设分析，不再强制使用方向规则。Writer/Reviewer 区分候选对照与因果消融证据。

- 实验报告在章节来源预算内优先保留所选方案引用的论文，不再固定重复检索列表前几篇；Writer/Reviewer 共用实际修改与代码复用、描述统计与显著性结论的表达边界。确定性分析不再将混合指标方向自动判为假设部分成立。

- 方案评估接收已引用阅读卡片的正文与来源关联限制，保留引用校验并展示具体失败原因；显式续跑可重新评估回退结果，不重复已完成的研究阶段，也不删除历史产物。

- 恢复研究会话的 Rich 启动面板、阶段说明、动态耗时和产物表，接通研究主链中的 CodeTask 进度消息；重定向日志保留静态阶段记录，不输出动画帧。耗时不代表训练百分比。
  检索展示服务商/查询与返回数量，阅读转发已有 LLM 批次消息，实验展示有限进程输出及日志目录。

- 研究 TOML 现在可以记录准备好的实验所选候选 idea，并将选择保留在 canonical
  session 中，避免依赖会话外手工修改。

- 显式的单次输出 token 上限现在具有最高优先级。推理输出设置只有在调用方和客户端都没有设置上限时才作为后备值，provider 的推理配置不会再静默抬高调用方的限制。

- 当已测指标方向唯一且不利、另一个声明指标不可用时，不再把假设误判为“部分支持”；结果保留为 `unsupported`，并保留缺失证据限制。

- canonical research CLI 现在转发报告 Writer/Reviewer 的阶段进度，长章节写作和审阅可以显示已有的检查点级消息，同时保持原有的有限恢复行为。

## 2026-09-14

- OpenAI 兼容 provider 请求默认限制为每次尝试 180 秒；显式设置
  `SIMPLE_AR_LLM_TIMEOUT_SEC=0` / `off` / `none` 仍可禁用客户端超时。输出 token
  仍默认不限制，除非用户主动配置。

- 分析阶段现在接收已选研究设计；没有设计产物时使用显式准备好的执行协议，保留假设、指标引用、证据引用和指标方向，不再把交接内容压缩成只有研究问题。

- 报告 Reviewer 现在会在有限的修订次数内处理轻微引用误用，以及指标事实不一致和无证据主张。

- Report Writer 与 Reviewer 现在都会收到已有的实验方案。配对报告保留产物中的逐种子详细记录，同时在正文附录展示紧凑的测量来源标签；回归测试区分报告中应审计的聚合指标与保留在产物中的原始测量。

- 添加配对测量时继续保留已选研究契约，使报告同时拥有假设和逐种子执行协议；有可用的候选标题时，实验报告使用候选标题，并在参考文献中呈现已有的作者与发表年份元数据。
...
- 显式选择候选后保留有限验证所需的设计契约及科学不确定性状态；没有选中契约时在 CodeTask 准备前暂停，避免先跑实验再因交接缺失失败。

- 研究 TOML 将显式全文/PDF 选项传给已有摄取链，高级模板开启全文；修复正式入口仅摄取摘要却无法配置全文的问题。

- 研究 TOML 接入已有配对实验命令与受保护协议，复用正式应用；完成后的续接不重跑矩阵。
- 研究综合区分用户目标与已经核验的实验准备条件，不再将自然目标自动描述为已准备环境。
- 新增 Mammoth/CIFAR-100 诊断适配，复用上游训练并保留逐任务指标来源；完整科研到论文验收仍待完成。

## 2026-09-13

- research-session 支持 --session-root：补齐执行条件后续接，保留已有研究证据与预算，不静默替换已有实验协议。
- 研究准备复用 CodeTask 工作区选项与自定义保护路径；auto 遇未提交源码时记录原因并使用 copy，不再悄悄只取 HEAD。

- 新增 research-session --config：复用 CLI 默认值与显式覆盖，支持文件相对路径、模型/输出上限、会话预算及轻量/完整 TOML 示例；缺少执行配置时仍保留实验交付目标。
- 支持显式授权后续额度，保留历史未知用量并限制新增消费；重载同一授权不会补回已用额度。
- 修复研究计划将含有 not 的用户目标整段当作排除主题的问题，继续支持显式范围排除。
- 报告审阅指出指标错误或无依据论断时，即使标为 minor，也在既有修订次数内交回 Writer 修正；提示性建议不自动触发重写。
- 删除按字数静默否决修订的规则，允许精简重复内容；后续 Reviewer 检查实际修订正文。
- 项目生成、审阅和修复复用同一结果schema构建函数，删除CodeTask执行器中的重复定义，默认指标及方向不变。
- 删除报告全文数字白名单推断，保留结构化指标归属校验及可见性检查；机械检查通过不代表正文语义成立，历史数字告警格式仍可读取。
- 最终报告审计采用各章最近一次有效审阅，完整历史意见保留在迭代记录中；审阅失败或回退不能清除旧问题，仍不宣称最终正文已完成语义核验。
- 报告格式重试区分无效响应与网络/预算失败，避免provider重试结束后又在报告层重复请求；显式回退仍说明未完成审阅，Writer格式恢复保留同一实现证据和指标表。
- CodeTask首次生成、恢复审阅、修复后审阅共用有限修复处理，保留既有审阅门槛和停止位置，删除重复分支与无效Reviewer参数传递。
- 补丁应用后和运行后审阅共用调用及缓存结果判断；修复恢复时仅凭文件存在跳过失败审阅的问题，通过或建议性审阅仍可复用，不重复调用模型。
- 报告提示中指标共用一组列名，保留全部数值、条件、单位与证据ID；移除重复Writer简报，不改变存档和工具读取格式。
- Writer/Reviewer共用执行证据投影现在包含冻结的实际patch及来源、截断标记；修复此前“工具可读但提示输入遗漏”的缺口，不把完整验证/审阅日志加入精简投影。
- 研究实现桥接一次进入CodeTask并停在验证，不再分别为计划、提案、验证重复执行准备路径。既有明确授权可继续待批准计划/提案；拒绝计划、交互默认行为、修改额度及应用层实验归属保持不变。
- 报告checkpoint保留等待首次审阅的已写草稿；Reviewer失败后恢复可直接复用草稿，不重复Writer调用，也不提前计作已审阅章节。
- CodeTask配置、CLI与执行器共用阶段顺序和审阅模式规范化，删除重复规则表；支持模式、默认值及配置/直接调用的错误类型不变。
- CodeTask明确区分修改额度授权与文件系统权限错误；补丁失败共用记录路径，研究桥接不再按错误文本中的budget重复判断或误报授权。
- 删除生成、修复、审阅中一律拒绝非ASCII标识符的规则及专用扫描模块；继续使用Python编译、接口和修改范围检查。审阅契约更新为13，旧审阅按既有恢复流程刷新，不重复实验。
- 已结束 attempt 的遗留用量预留转为 unknown 并保留预算占用，恢复不会把未测得的调用消耗退回额度。
- Writer/Reviewer 使用精简指标投影，移除重复存档路径和协议哈希，不再仅取前12项；完整来源仍保存在报告输入与指标读取工具中。
- 报告 checkpoint 按模板/审阅标准的内容判断输入一致性，不再受安装路径变化影响；保留来源路径记录。新 checkpoint 使用修正后的标识，不追写历史快照。
...
- ARC-Bench `summarize` 现在会在汇总 refresh variant 时自动合并 source run 的 init/execute 成本，`Total Time` / 总 token 表示完整实验口径，同时保留 `Postprocess` / `Postprocess Tokens` 表示本次重算增量。
