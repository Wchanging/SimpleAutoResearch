...
- **研究源规划**：每次 `02-search` 会写入 `research_questions.json`、`query_plan.json` 和 `source_plan.json`，记录 OpenAlex/Semantic Scholar/arXiv/本地文件源、可选 LLM-backed query planning、facet-driven query expansion、retrieval-round traces、screening decisions、cache 策略和轻量预算。
...
- 文献检索现在会写入可审计的 source plan，并支持 OpenAlex、Semantic Scholar、arXiv 和本地 Markdown/text 笔记，但还不是完整 PDF 解析、parser-backed 或向量 RAG survey 系统。
